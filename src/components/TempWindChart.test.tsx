...
describe('TempWindChart', () => {
  ...
  it('renders without crash', () => {
    expect(() =>
      render(<TempWindChart data={mockData} xAxisMode="clock" onHoverDistance={vi.fn()} />)
    ).not.toThrow();
  });

  it('shows unavailability message when weatherAvailable is false', () => {
    const { getByText } = render(
      <TempWindChart data={mockData} xAxisMode="clock" onHoverDistance={vi.fn()} weatherAvailable={false} />
    );
  ...
  it('does not show unavailability message when weatherAvailable is true', () => {
    const { queryByText } = render(
      <TempWindChart data={mockData} xAxisMode="clock" onHoverDistance={vi.fn()} weatherAvailable={true} />
    );
